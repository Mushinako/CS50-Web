...
interface simpleFetchResponseMap {
    GET: emailInfo | emailInfo[],
    POST: sendEmailResponse,
    PUT: setEmailStatusResponse,
}
...
function createEmailHeaderRow(rowClass: string, headerText: string, cellText: string): HTMLTableRowElement {
...
function createEmailPreviewDiv(email: emailInfo, mailbox: mailboxType): HTMLDivElement {
...
function createEmailDetailsHeaderDiv(email: emailInfo): HTMLDivElement {
...
function createEmailDetailsActionsDiv(email: emailInfo): HTMLDivElement {
...
function createEmailDetailsBodyDiv(email: emailInfo): HTMLDivElement {
    const bodyDiv = createElement("div", {
        text: email.body,
        classes: ["email-body"],
    });
...
function createEmailDetailsDiv(email: emailInfo): HTMLDivElement {
    const emailDiv = createElement("div", {
...
async function simpleFetch<M extends keyof simpleFetchResponseMap>(url: string, method: M, headers?: HeadersInit): Promise<potentialErrorResponse<simpleFetchResponseMap[M]>> {
    const response: potentialErrorResponse<simpleFetchResponseMap[M]> = await fetch(url, {
        method: method,
        headers: headers,
    })
